      ...
      {isLoading || !s ? <div className="glass p-6"><div className="skeleton h-32 w-full" /></div> : (
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="glass p-5"><div className="card-label">TOTAL INVESTED</div><div className="num mt-2 text-2xl font-bold">{inr(s.capital)}</div></div>
            <div className="glass p-5"><div className="card-label">ESTIMATED PROFIT</div><div className={`num mt-2 text-2xl font-bold ${s.estProfit >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>{money(s.estProfit)}</div><div className="mt-1 text-[11px] text-slate-500">Estimate — not guaranteed</div></div>
            <div className="glass p-5"><div className="card-label">EST. PORTFOLIO VALUE</div><div className="num mt-2 text-2xl font-bold">{inr(s.estValue)}</div></div>
          </div>
          <div className="glass overflow-x-auto">
